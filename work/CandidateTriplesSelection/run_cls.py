...
#     batch_q = [] 
#     batch_attr = [] 
#     with open('./data/dev_data_cls.tsv','r',encoding='utf-8') as lines:
#         for line in lines:
#             data = line.strip().split('\t')
#             golds.append(int(data[-1]))
#             batch_q.append(data[0])
#             batch_attr.append(data[1])
#             if len(batch_q) == 8 :
#                 pred_labels =  cls_predict(cls_model,cls_tokenizer,batch_q,batch_attr)
#                 preds.extend([int(p) for p in pred_labels])
#                 batch_q = [] 
#                 batch_attr = []
#     if len(batch_q)>0:
#         pred_labels =  cls_predict(cls_model,cls_tokenizer,batch_q,batch_attr)
